 ...
 *
 * Four knobs, 2×2. ECHO is a placeholder: it holds a value and is saved like
 * the others, but nothing reads it yet — no sound, no drawing — until a delay
 * is built for it.
 *
...
export type FxControlId = 'reverb' | 'highpass' | 'saturate' | 'echo'
...
export type FxControl = { id: FxControlId; label: string; macroName?: string; hint: string }
...
export const FX_CONTROLS: FxControl[] = [
  {
    ...
    label: 'REVERB',
    macroName: 'Reverb',
    hint: 'Reverb — the room around every sound. Drag up or down',
  },
  ...
  // bridge matches the macro name literally, so this must be Live's spelling.
  {
    ...
    macroName: 'Hi Pass Filter',
    hint: 'High-pass filter — thins out the low end. Drag up or down',
  },
  ...
  // moved with the meaning rather than leaving a name that describes nothing.
  {
    ...
    label: 'SATURATE',
    macroName: 'Saturate',
    hint: 'Saturate — adds grit and drive. Drag up or down',
  },
  // Placeholder — no macro and no drawing yet. See the note at the top.
  { id: 'echo', label: 'ECHO', hint: 'Echo — not connected yet' },
]
